...
Para isso, iremos mexer com imagens **.JPGE**.

## Imagens **.JPGE**

Antes de iniciarmos a brincadeira, vale uma breve introdução às imagens **.JPGE**, pois elas formarão os `data.frame`'s do laboratório.

Arquivos com extenção *.jpge* guardam 5 coordenadas que são suficientes para serem entendidas e desenhadas pelo computador:
...
1. Para construir nossos bancos de dados, carregue as duas imagens abaixo (clique para download e salve na pasta do seu projeto):
    - [purple_wave.jpeg](http://curso-r.github.io)
    - [xadrez_colorido.jpeg](http://curso-r.github.io)
2. Com auxílio da função `readJPEG()` do pacote `jpeg`, carregue uma das imagens no R e transforme para `data.frame` (primeiro a *purple_wave.jpge*).
